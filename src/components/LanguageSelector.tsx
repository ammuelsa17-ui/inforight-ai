...
export default function LanguageSelector() {
  ...
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#F4F9FF] border border-[#BCD7EE] text-xs text-[#102A56] shrink-0">
      <Globe className="w-3.5 h-3.5 text-[#4F46E5] shrink-0" />
      <select
        value={selectedLanguage}
        onChange={(e) => setSelectedLanguage(e.target.value as BharatLanguageCode)}
        className="bg-transparent font-semibold focus:outline-none cursor-pointer text-[#102A56] min-w-0 max-w-[120px] sm:max-w-[170px]"
